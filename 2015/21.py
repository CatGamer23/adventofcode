...
def part1(data: list[str]) -> str | int | float | None:
  ...
  def simulate_battle(player_damage: int, player_armor: int) -> bool:
    player_turns_to_win: int = -(-boss_hp // max(1, player_damage - boss_armor))  # noqa
    boss_turns_to_win: int = -(-player_hp // max(1, boss_damage - player_armor))  # noqa
  ...
  for weapon, armor, (ring1, ring2) in product(items[:5], items[5:10], combinations(items[10:], 2)):
